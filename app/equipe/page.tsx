const teamMembers = [
  ...
  {
    name: "Dr Simon MATHE",
    role: "Médecine générale • Médecine du sport",
    gender: "male",
    icon: "🩺",
  },
  {
    name: "Dr Pierre-Etienne LECHANTRE",
    role: "Médecine générale • Sommeil",
    gender: "male",
    icon: "🩺",
    ...
    <main className="min-h-screen bg-[#F8FAF9] text-slate-800">
      <section className="bg-gradient-to-br from-[#E8F1EB] via-white to-[#E8EEF6] px-6 py-24">
        <div className="max-w-7xl mx-auto">
          <a href="/" className="text-[#5D89AD] font-medium">
            ← Retour à l’accueil
          </a>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {teamMembers.map((member) => (
              <div
                key={member.name}
               className="group bg-white rounded-[32px] p-8 border border-slate-100 shadow-sm hover:shadow-2xl transition duration-300 overflow-hidden"
              >
                <div className="flex items-start gap-6">
                  <div className="relative w-28 h-28 shrink-0 rounded-[32px] bg-gradient-to-br from-[#DCE8DF] to-[#DCE5EF] flex items-center justify-center shadow-inner">
                   {member.image ? (
  <img
  src={member.image}
  alt={member.name}
 className="w-full h-full object-cover rounded-[32px] transition duration-700 ease-out group-hover:scale-135 group-hover:rotate-1"
/>
) : (
